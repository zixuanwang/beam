...
namespace Beam{
	WavWriter::WavWriter(const std::string& file, int sample_rate, short channels) : m_sample_rate(sample_rate), m_channels(channels), m_size(0){
		...
		write<short>(m_out_stream, 1);                             // Format
		write<short>(m_out_stream, m_channels);                    // Channels
		write<int>(m_out_stream, m_sample_rate);                   // Sample Rate
		write<int>(m_out_stream, m_sample_rate * m_channels * 2);  // Byterate
		write<short>(m_out_stream, 2);                             // Frame size
		write<short>(m_out_stream, 16);                            // Bits per sample
