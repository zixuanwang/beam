...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		for (int channel = 0; channel < MAX_MICROPHONES; ++channel) {
			frequency_input[channel].assign(FRAME_SIZE,
		...
		Beam::Pipeline::instance()->beamforming(frequency_input,
				beamformer_output); // beamforming
		...
		// copy half frame.
		for (int channel = 0; channel < MAX_MICROPHONES; ++channel) {
			input_prev[channel].assign(input_current[channel].begin(),
					input_current[channel].end());
		}
	}
	delete[] buf;
	delete[] output_buf;
